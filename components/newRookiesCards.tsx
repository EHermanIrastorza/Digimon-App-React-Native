...
export default function NewRookiesFilterCards() {
    const [rookies, setRookies] = useState<IdigimonNew[]>([]);
    ...
    useEffect(() => {
        const fetchChampions = async () => {
            ...
            const filteredRookies = data.filter((digimon) => digimon.level.toLocaleLowerCase() === "rookie")
    ...
    const paginateData = rookies.slice(
        (currentPage - 1) * itemsPerPage,
        currentPage * itemsPerPage
    );

    const totalPages = Math.ceil(rookies.length / itemsPerPage)
    return (
